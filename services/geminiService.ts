 ...
 * `true`: Generates both creative art and a brief explanation.
 * `false`: Generates only the creative ASCII art.
 */
const ENABLE_ASCII_TEXT_GENERATION = true;
...
export async function* streamDefinition(
  ...
  const prompt = `In the context of K-12 education and school development, provide a concise, single-paragraph encyclopedia-style definition for the term: "${topic}". The definition should be grounded in established educational theory and research. Respond with only the definition. Do not include a reference list, in-text citations, prominent scholars, or any other special formatting.`;
...
export async function generateAsciiArt(topic: string): Promise<AsciiArtData> {
  ...
  const textPromptPart = `2. "text": A brief, one-sentence, poetic explanation of how the art visualizes "${topic}".`;

  const keysDescription = `two keys: "art", "text"`;
  const promptBody = `${artPromptPart}\n${textPromptPart}`;

  const prompt = `For "${topic}", create a JSON object with ${keysDescription}.
${promptBody}

Return ONLY the raw JSON object, no additional text. The response must start with "{" and end with "}" and contain only the requested properties.`;
